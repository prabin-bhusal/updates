    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg flex justify-evenly">
            <div class="p-6 text-gray-900">
                ...
                <p class="mt-6 mb-6 text-lg font-normal text-gray-500 max-w-4xl">
                    {{ $event->content }}
                </p>
            </div>
            <div class="p-6 text-gray-900 basis-1/4">
                <h2 class="text-4xl font-extrabold pb-4 mb-4 border-b-2 border-gray-100">Event details</h2>
                <p class="mb-6 text-lg font-normal text-gray-500">
                    <span class="font-semibold">Posted on:</span> {{ Str::limit($event->created_at, 10, '') }}
                    <br />
                    @can('updateByAdmin', $event)
                    <span class="font-semibold">Posted by:</span> {{ $event->admin->name }}
                    @endcan
                    @can('updateByUser', $event)
                    <span class="font-semibold">Posted by:</span> {{ $event->user->name }}
                    @endcan
                </p>
            </div>
        </div>
